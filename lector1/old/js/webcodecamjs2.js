...
var WebCodeCamJS = function(element) {
    ...
    function buildSelectMenu(selectorVideo, ind) {
        x = document.getElementById("mensaje").innerHTML;
        x += 'entró<br>';
        videoSelect = Q(selectorVideo);
        x += videoSelect.selectedIndex+'<br>';
    ...
    function gotSources(device) {
        if (device.kind === 'video' || device.kind === 'videoinput') {
            ...
            if (face.indexOf("back")>=0) {
                var msj = 'lo consiguió<br>'+document.getElementById("camera-select").value+'<br>';
                msj += device.deviceId+"<br>"+device.label+"<br><br>";
                document.getElementById("mensaje").innerHTML = msj;
                html('<option value="' + (device.id || device.deviceId) + '" selected>' + text + '</option>', videoSelect);
            }
        }
    }
